...
export interface UserUpdateRoleCommand {
  execute(account: Account, role: Role): Promise<Result<Account, AppError>>;
}

export class UserUpdateRoleCommandImpl implements UserUpdateRoleCommand {
  constructor(private readonly accountRepository: AccountRepository) {}

  async execute(account: Account, role: Role): Promise<Result<Account, AppError>> {
    const updatedAccount = account.update(undefined, undefined, undefined, undefined, role);
